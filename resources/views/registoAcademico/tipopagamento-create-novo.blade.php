...
<th>Acções</th>
        ...
        <tbody class="boddy">
            @foreach ($dados as $key=> $dadosItem )

<tr>
    <td>{{$key+1 }}</td>
    <td> <i class="fa fa-lg {{ $dadosItem->icon }}"></i></td>
    <td>{{ $dadosItem->Descricao }}</td>
    <td class="action-btns">
        <button class="btn btn-outline-primary btn-edit edit-tipoPagamento btn-sm"
        title="{{ $dadosItem->Descricao }}" id="{{ $dadosItem->id }}"  icon="{{ $dadosItem->icon }}"  tipo_janela="{{$dadosItem->tipo_janela}}" >
         <i class="fa fa-edit "></i></button>

         <button class="btn btn-outline-danger btn-trash apagarNovo btn-sm"
        title="{{ $dadosItem->Descricao }}" id="{{ $dadosItem->id }}"  >
         <i class="fa fa-trash "></i></button>
        {{--  <button class="btn btn-danger btn-apagar"  > <i class="fa fa-trash"></i></button>  --}}
...
const icon = $("[name='icon']").val();
           $pagamento= $(".nomePagamento").val();
            $.ajax({
                url:"{{Route('tipoPagamento.guardar') }}",
                type: 'POST',
    data: {
    'nomePagamento': $pagamento,
	'icon':icon,
    ...
    const icon = $("[name='icon']").val();
             $.ajax({
                 url:"{{Route('tipoPagamento.atualizar') }}",
                 type: 'POST',
     data: {
     'nomePagamento': $pagamento,
     'id':$id,
     ...
     success: function(data){
        location.reload(true);
         ...
         $(".apagarNovo").click(function(){
           ...
           $id= $(this).attr('id');
             $.ajax({
                 url:"{{Route('tipoPagamento.apagar') }}",
                 type: 'POST',
     data: {
     'nomePagamento': $pagamento,
     'id':$id,
