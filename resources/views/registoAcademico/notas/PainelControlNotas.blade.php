                            ...
                            <div class="form-group col">
                                ...
                                <select class="form-control controledeclasseano" name="anolectivo" id="anolectivo">
                                    @foreach ($anolectivo as $ano)
                                        <option value="{{ $ano->id }}"   @if($ano->anolectivo ==carbon\Carbon::now()->format("Y")) selected=true @endif>
                                            {{ $ano->anolectivo }}
                                        </option>
                                    @endforeach
                                </select>
...
function getpainelConfiguracoes(ano,classe){
    let urlBase = "{{ route('notas.ConfiguracoesTrimestraisano', [':ano' ,':classe']) }}";
...
$(document).on('click', '.btngravaralteracoesTrimestres', function () {
    ...
    $.ajax({
        url: "{{ route('notas.TrancarTrimestreAll') }}", // rota nomeada
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}", // token CSRF obrigatório
