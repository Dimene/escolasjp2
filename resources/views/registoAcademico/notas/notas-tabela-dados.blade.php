...
{{-- {{ dd($dadosbloqueio->Estado1 ??) }} --}}
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="row">
                    ...
                    @if(empty($dadosbloqueio)||$dadosbloqueio->Estado1==null)
            <button class="btn btn-primary col" data-toggle="modal" data-target="#formulaModal">
                <i class="fas fa-calculator"></i> Configurar Média
            </button>
            @endif

            <a class="btn btn-success col" href="{{ Route('notas.export', [$iddisciplia, $turmaid, $trimestre]) }}">
...
<th>
    <div class="d-flex justify-content-between align-items-center">
        <span class="notanome">{{ $dispv->nota_descricao }}</span>
        ...
        @if($dispv->nota_meta_id != 0)
            ...
            <button class="btn btn-sm btn-primary btn-column-action editar-notaavaliacao"
                provaid="{{ $dispv->nota_meta_id }}"
                disciplina_id="{{ $iddisciplia }}"
                turma_id="{{ $turmaid }}"
                trimestre="{{ $trimestre }}"
                title="Editar">
            ...
            <!-- Botão Excluir -->
           {{-- <button class="btn btn-sm btn-danger btn-column-action apagar-notaavaliacao"
                provaid="{{ $dispv->nota_meta_id }}"
                disciplina_id="{{ $iddisciplia }}"
                turma_id="{{ $turmaid }}"
                trimestre="{{ $trimestre }}"
        ...
        @endif
            ...
            <tbody>
                ...
                <tr data-aluno-id="{{ $turmaItem['id'] }}">
                    <td>{{ $key + 1 }}</td>
                    <td class="font-weight-bold">{{ $turmaItem['nome'] }}</td>
                    <td class="font-weight-bold">{{ $turmaItem['sexo'] }}</td>
                    ...
                    @foreach ($disciplinaavalaicaotipo[$iddisciplia] as $index => $dispv)
                        @if ($dispv->nota_meta_id != 0)
                            ...
                            {{-- {{ dd($turmaItem['disciplinas']["idnotas"]["chave1"],$turmaItem['disciplinas']["idnotas"]["chave"]) }} --}}

                            class="@if(empty($turmaItem['disciplinas']["idnotas"]["chave1"])&&empty($turmaItem['disciplinas']["idnotas"]["chave2"]))
                             editable @endif"

                            id="{{ $key."".$index }}"
                                data-aluno="{{ $turmaItem['id'] }}"
                                data-avaliacao="{{ $dispv->nota_meta_id }}"
                                 avaliacaobvalor="{{ $turmaItem['disciplinas'][$iddisciplia][$dispv->nota_meta_id] ?? '' }}"
                                data-avaliacaoNome="{{ $dispv->nota_descricao}}"
                                 {{ $dispv->nota_meta_id }}="{{ $turmaItem['disciplinas'][$iddisciplia][$dispv->nota_meta_id] ?? 0 }}"
                                data-idnota="{{ $turmaItem['disciplinas']['idnotas'][$dispv->nota_meta_id] ?? '' }}">
                                {{ $turmaItem['disciplinas'][$iddisciplia][$dispv->nota_meta_id] ?? '' }}


                            </td>
                        @else
                            <td class="media-cell" id="media-{{ $turmaItem['id'] }}">
                                {{ $turmaItem['disciplinas'][$iddisciplia][$dispv->nota_meta_id] ?? '' }}
                            </td>
                        @endif
                    @endforeach
                ...
                <form id="formulaForm">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control" id="formulaInput" name="formmulacriada"
                                value="{{ !empty($formunlamendia) ? $formunlamendia->formula : '' }}">
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="d-flex flex-wrap">
                            @foreach ($disciplinaavalaicaotipo[$iddisciplia] as $nota_metaItm)
                                @if ($nota_metaItm->nota_meta_id != 0)
                                    <button type="button" class="btn btn-outline-primary m-1 btn-insert-var"
                                        data-var="{{ $nota_metaItm->nota_descricao }}">
                                        {{ $nota_metaItm->nota_descricao }}
                                    </button>
                                @endif
                            @endforeach
                        </div>
