...
classe_disciplinas.forEach(function(e) {
    ...
    let divisao = window.trimestre[0].divisao??null;
        ...
        success: function(dados) {
            if (dados.alert === "success") {
                $('#listadosalunos tbody tr').each(function() {
                    ...
                    guardarmediaAnual($celula); // aplica a função na célula correta
...
function guardarmediaAnual($celula) {
    ...
    $.ajax({
        url: "/RegistoAcademico/notas/trimestre/guadarMediaAnual/",
        type: "post",
        dataType: "json",
        data: {
            "_token": $("[name='_token']").val(),
...
function Resultado($celula) {
    ...
    let contar = 0;
    let temNotaMuitoBaixa = false;

    const $linha = $celula.closest("tr");

    // pegar objeto da linha no DataTable
    ...
    $linha.find("td[siglaDisp]").each(function () {
        ...
        if (!isNaN(nota)) {
            ...
            // regra nota muito baixa
          if ((nota >= 0 && nota <= 7)) {
                temNotaMuitoBaixa = true;
            }
    ...
    // reprova direto
    if (temNotaMuitoBaixa) {
        resultado = "Reprovad";
    }

    // regra das 3 disciplinas
    if (resultado === "Aprovad" && contar > 2) {
        resultado = "Reprovad";
    }

    // genero
    const sufixo = sexo === "F" ? "a" : "o";
    const resultadoFinal = resultado + sufixo;
...
if(estado=== "on") {
    guardarmediaAnual($(this)); // aplica a função na célula correta
...
$(document).on('click', '.BaixarExcel', function () {
    ...
    let corpo = $tabelatrimestral.rows().data().toArray();
...
function mediapordisciplina($celula) {
    ...
    $('td[linha="' + linha + '"][disciplina="' + disciplina + '"]').each(function () {
        ...
        formula = formula.replace(nt + "NF", nota);
        console.log(formula);
...
classe_disciplinas.forEach(function(e) {
    ...
    let divisao = trimestre[0].divisao;
    let totalLinhas = $dadosparaEnvio.length; // `.count()` não existe em JS puro
...
function uploadExcel(files) {
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log("Arquivo enviado com sucesso:", response.dados);

            $tabelatrimestral.rows().every(function (rowIdx) {
                const novaData = response.dados[rowIdx];
                $tabelatrimestral.row(rowIdx).data(novaData).invalidate(); // força redraw da célula

                var linha = $(this.node()); // pega o <tr> corretamente
                var idAluno = linha.attr('dadosnota');

                guardaLinhaBD(idAluno, organisarArraySava(linha));
            });

            $tabelatrimestral.draw(false); // redesenha uma única vez após atualizar todas as linhas

            // Se estiver usando plugin de edição, reaplique aqui
            $tabelatrimestral.$('td.editavel').editable?.(); // se estiver usando jEditable
