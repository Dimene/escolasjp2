...
@section('title', 'notas dos alunos')
@section('content')
...
<div class="card-body row">
          ...
          <select id="my-classe" class="form-control controler-classe-ano" name="">
                        ...
                        @foreach ($class as $classItem)
                            @foreach ($classesprofessor as $item)
                                @if ($classItem->id == $item->classe_id && !empty($classItem->Exame))
                                    <option value="{{ $classItem->id }}">{{ $classItem->Descricao }}</option>
                                @endif
                            @endforeach
                        @endforeach
                    ...
                    <select id="my-anolectivo" class="form-control controler-classe-ano" name="">
                        @foreach ($anolectivo as $anolectivoItem)
                            @foreach ($anoslectivoprofessore as $anoslectivoprofessorItem)
                                @if ($anoslectivoprofessorItem->ano_lectivo_id == $anolectivoItem->id)
...
buscar_turmas(anolectivo, classe);
 ...
 $(document).on("click", ".turmaselecionada", function () {
    ...
    $(".turmaselecionada").removeClass("active");

    // ativa o clicado
    $(this).addClass("active");

    // pega dados do elemento clicado
    let id = $(this).attr("id");
    let flag = $(this).attr("flag");

    buscar_turmaSelecionada(id, flag);
});
...
function buscar_turmas(ano, classe) {
                ...
                $.ajax({
                    url: '/RegistoAcademico/notas/turmasTrimestre/' + ano + '/' + classe + '/'+1,
                    type: "GET",
...
let id = turma.id;
let flag = turma.getAttribute("flag");
...
buscar_turmaSelecionada(id, flag);
            ...
            function buscar_turmaSelecionada($id,$flg) {


                $.ajax({
                    url: "/RegistoAcademico/notas/disciplinas/PautadeExame/"
                        +$id + "/"+$flg,
                    type: "GET",
                    beforeSend: function() {
                        $(".bodynotas").empty();
    ...
    <div class="d-flex justify-content-center align-items-center" style="height: 50px; align:center">
        <img src="{{ asset('imageproceaament/loading.gif') }}"
             alt="Carregando..."
...
$(document).on("click", ".jurrisMostrar", function () {



    let id = $(".listaJurriTab > li >a.active").attr("id");
    let flag = $(".listaJurriTab > li >a.active").attr("flag");

     $turma=id;
    $flagturma=flag;
      $(".listaJurriTab").fadeToggle();
    $(".listaTurmaTab").fadeToggle();

    buscar_turmaSelecionada(id,flag);

});

$(document).on("click", ".turmasMostrar", function () {

    $(".listaJurriTab").fadeToggle();
    $(".listaTurmaTab").fadeToggle();

    let id = $(".listaTurmaTab > li>a.active").attr("id");
    let flag = $(".listaTurmaTab > li>a.active").attr("flag");
