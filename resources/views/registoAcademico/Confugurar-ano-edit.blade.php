...
@section('title', 'Actualizar Ano Lectivo')
@section('content')
    ...
    <div class="container-fluid fade-in">
        ...
        @if (Session::get('mensage'))
            <div class="alert alert-success-modern alert-modern">
                <i class="fa fa-check-circle mr-2"></i>
                <strong>Sucesso!</strong> {{ Session::get('mensage') }}
            </div>
        @endif
        ...
        <div class="row">
                        ...
                        <div class="list-group-modern">
                            @foreach ($anoLectivo as $anoLectivoItem)
                                <div class="list-group-item-modern">
                                    <div>
                                        <span class="ano-info">{{ $anoLectivoItem->anolectivo }}</span>
                                        <span class="ano-modelo">({{ $anoLectivoItem->anomodelo->Descricao }})</span>
                                    </div>
                                    <div class="action-badges">
                                        <a href="{{ Route('ano.edit', $anoLectivoItem->id) }}"
                                            class="badge-action badge-edit">
                                            <i class="fa fa-edit"></i> Editar
                                        </a>
                                        <a href="{{ Route('ano.show', $anoLectivoItem->id) }}"
            ...
            <div class="col-lg-8">
                <div class="form-section">
                    ...
                    <form action="{{ Route('ano.update', $anoLectivoEscolhido->id) }}" method="POST">
                        @method('PUT')
                        @csrf
                            ...
                            <div class="col-12">
                                <div class="form-group-modern">
                                    ...
                                    <select class="select-modern" name="anolectivo" required>
                                        @for ($i = 0; $i < 10; $i++)
                                            <option value="{{ 2020 + $i }}"
                                                @if (2020 + $i == $anoLectivoEscolhido->anolectivo) selected @endif>
                                                {{ 2020 + $i }}
                                            </option>
                                        @endfor
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group-modern">
                                    ...
                                    <input type="date" class="input-modern" name="Inicio_ano"
                                        value="{{ $anoLectivoEscolhido->Inicio ?? '' }}" required>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group-modern">
                                    ...
                                    <input type="date" class="input-modern" name="Fim_ano"
                                        value="{{ $anoLectivoEscolhido->Fim ?? '' }}" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="division-section">
                            ...
                            <div class="form-group-modern">
                                ...
                                <select class="select-modern modalidadeV" name="modalidade" required>
                                    <option value="3" @if ($anoLectivoEscolhido->modalidade == '3') selected @endif>Trimestre (3
                                        períodos)</option>
                                    <option value="2" @if ($anoLectivoEscolhido->modalidade == '2') selected @endif>Semestre (2
                                        períodos)</option>
                                    <option value="4" @if ($anoLectivoEscolhido->modalidade == '4') selected @endif>Bimestre (4
                                        períodos)</option>
                                    <option value="12" @if ($anoLectivoEscolhido->modalidade == '12') selected @endif>Modular (12
                                        módulos)</option>
                                </select>
                            </div>
                            ...
                            <div id="divisionContainer">
                                ...
                                @if ($divisoes->isEmpty())
                                    <div class="division-row ">
                                        ...
                                        @foreach ($divisoes as $index => $divisao)
                                            <div class="division-row {{ $index > 0 ? 'DIVISAOaCRECIDA' : '' }}">
                                                <div>
                                                    <input type="hidden" name="idDivisao[]"
                                                        value="{{ $divisao->id ?? '' }}">
                                                    <input type="number" class="input-modern" name="numero[]"
                                                        placeholder="Nº" value="{{ $divisao->divisao }}" required>
                                                </div>
                                                <div>
                                                    <input type="date" class="input-modern" name="Inicio[]"
                                                        value="{{ $divisao->Inicio }}" required>
                                                </div>
                                                <div>
                                                    <input type="date" class="input-modern" name="Fim[]"
                                                        value="{{ $divisao->Fim }}" required>
                                                </div>
                                                <div>
                                                    @if ($index == 0)
                                                        <button type="button" class="btn-add addicionar-mais-Divisao">
                                    ...
                                    <input type="hidden" class="controlerleght" value="{{ count($divisoes) }}">
                            </div>
