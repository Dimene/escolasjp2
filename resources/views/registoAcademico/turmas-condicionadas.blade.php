...
<div class="container mt-4">
    <div class="row">
        ...
        @foreach ($dadosTurmas as $key => $turma)


            <div class="col-md-6 mb-4 card-turma" id="card-{{ $key }}">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white" style="cursor: pointer;" onclick="toggleTurma('turma-{{ $key }}')">
                        <h5 class="mb-0 d-flex justify-content-between align-items-center">
                            ...
                            <input name="turmasNome[]" class="form-control form-control-sm w-50 d-inline-block" value="{{ $turma[0]->turma ?? $alfabeto[$key] }}">
                            <span class="toggle-icon">⬇</span>
                        </h5>
                    </div>
                    <div class="card-body turma-table" id="turma-{{ $key }}" style="display: none;">
                        <table class="table table-bordered listadosalunosTurma" id="turmaAlunos-{{ $key }}">
                            ...
                            <tbody>
                                @foreach ($turma as $aluno)
                                    ...
                                    <tr id="aluno-{{ $aluno->id }}">
                                        ...
                                        <td>{{ $aluno->idade }}</td>
                                        <td>
                                            <select class="form-control" onchange="moverAluno({{ $aluno->id }}, '{{ $key }}', this.value)">
                                                <option value="">{{$tituloselecao }}</option>
                                                @foreach ($dadosTurmas as $key2 => $turmaDestino)
                                                    @if ($key !== $key2)
                                                        <option value="{{ $key2 }}">{{  $optionturma }} {{ $turmaDestino[0]->turma ?? $alfabeto[$key2] }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        </td>
...
// }
    ...
    $.ajax({
        url: "{{ route('turma.guardarTodasTurmas') }}",
        method: 'POST',
        dataType: 'json',
        data: {
