...
@section('title', 'Editar Tabela de Valores')
...
<div class="">
    ...
    <div class="container-fluid fade-in">
        <div class="row">
            <div class="col-12">
                @include("Componetes.menu-componte-tabelavalores")
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card-modern">
                    ...
                    <div class="p-4" style="background: white;">
                        <form id="formTabelaValores" action="{{ route('TabelaValores.update', $tabelaItem->id) }}" method="POST">
                            @csrf
                            @method("PUT")
                            ...
                            <div class="row">
                                        ...
                                        <select class="select-modern" id="my-select-AnoLectivo" name="AnoLectivo"
                                            ...
                                            @foreach ($anolectivo as $anolectivoItem)
                                                <option value="{{ $anolectivoItem->id }}"
                                                    @selected($tabelaItem->anolectivo_id == $anolectivoItem->id)>
                                        ...
                                        <select class="select-modern" id="my-Descricao" name="Descricao"
                                            ...
                                            @foreach ($tipopagamento as $item)
                                                <option value="{{ $item->id }}"
                                                    @selected($tabelaItem->Descricao == $item->Descricao)>
                                                    {{ $item->Descricao }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group-modern">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text"
                                                   ...
                                                   name="Montante"
                                                   class="input-modern"
                                                   value="{{ number_format($tabelaItem->valorDescricao, 2, ',', '.') }}">
                                        </div>
                                            ...
                                            <label class="list-group-item-modern checkbox-custom d-flex align-items-center justify-content-between">
                                                <div>
                                                    <input type="checkbox"
                                                           ...
                                                           name="classes[]"
                                                           value="{{ $classe->id }}"
                                                           @checked($classeporValor->pluck('classe_id')->contains($classe->id))
                                                           @disabled($classescomDados->pluck('classe_id')->contains($classe->id))>
                                                    <span class="checkmark-custom"></span>
                                                    <strong>{{ $classe->Descricao }}</strong>
                                                </div>
                                                @if($classescomDados->pluck('classe_id')->contains($classe->id))
                                ...
                                <div class="col-lg-4 mb-4">
                                    ...
                                    <div style="max-height: 380px; overflow-y: auto;" class="scrollable-content">
                                        @include("Componetes.modalidade-pagamento")
                                    </div>
                                </div>
                            </div>
...
$(function(){
    ...
    $(".guardarpreco").on("click", function(e){
        ...
        // Enviar AJAX
        $.ajax({
            url: "{{ route('TabelaValores.update', $tabelaItem->id) }}",
            type: "POST",
            data: data,
