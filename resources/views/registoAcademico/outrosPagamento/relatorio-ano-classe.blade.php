            ...
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                @php
                  ...
                  $geral = 0;
                @endphp
                @foreach ($arrayrelatorio as $item)
                  @php
                  ...
                  <tr>
                    <td>
                      ...
                      <a href="{{ route('mensalidade.Relatoriodetalhado', [$ano, $classe, $item['mesId']]) }}" class="btn btn-sm btn-link p-0 ml-2" title="Ver detalhes">
                        <i class="fas fa-external-link-alt"></i>
                      </a>
                    </td>
                    <td>{{ $item['NrAlunos'] }}</td>
                    <td>{{ $item['tranferencias'] ?? 0 }}</td>
                    <td>{{ $item['disistentes'] ?? 0 }}</td>
                    <td>{{ $item['qtdQpagar'] ?? 0 }}</td>
                    <td>{{ $item['alunoscommulta'] ?? 0 }}</td>
                    <td class="text-success">{{ number_format($valorMulta, 2, ',', '.') }} MT</td>
                    <td class="text-primary">{{ number_format($valorMensal, 2, ',', '.') }} MT</td>
                    <td class="font-weight-bold">{{ number_format($totalLinha, 2, ',', '.') }} MT</td>
                  </tr>
                @endforeach
              </tbody>
              <tfoot class="total-row">
                <tr>
                  ...
                  <th class="text-success">{{ number_format($totalMultas, 2, ',', '.') }} MT</th>
                  <th class="text-primary">{{ number_format($totalMensalidades, 2, ',', '.') }} MT</th>
                  <th class="font-weight-bold">{{ number_format($geral, 2, ',', '.') }} MT</th>
                </tr>
              </tfoot>
            </table>
          </div>
...
<script>
  ...
  const dadosMeses = @json($arrayrelatorio);
