                            ...
                            <div class="form-group">
                                ...
                                <select id="selectAno" class="form-control selectAno" name="ano">
                                    @for ($x = 0; $x < count($anos); $x++)
                                        <option value="{{ $anos[count($anos) - 1 - $x]->id }}">
                                            📅 {{ $anos[count($anos) - 1 - $x]->anolectivo }}
                                        </option>
                                    @endfor
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="selectClasse" class="form-control selectClasse" name="classe">
                                    @foreach ($claases as $claasesItem)
                                        <option value="{{ $claasesItem->id }}">📚 {{ $claasesItem->Descricao }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    $("#uploadForm").on('submit', function(e) {
        ...
        var ano = $(".selectAno").val();
        var urlup = "{{ route('aluno.uplodefileCadastrofile', ':ano') }}";
    ...
    $(document).on("click", ".tranferir_Desistencia", function() {
        ...
        let avatar = "{{ asset('storage/' . $subdomain . '/fotoAluno') }}/" + foto;
    ...
    $(document).on("click", ".visualizar", function() {
        ...
        $.ajax({
            ...
            type: "GET",
            beforeSend: function() {
                const loadingGif = '{{ asset("imageproceaament/loading.gif") }}';
                $(".DadosAlunoBody").html(`
...
function atualizarUrls(ano) {
    var urlDownload = "{{ route('aluno.planilhaCadastro', ':ano') }}";
