...
<form method="POST" action="{{ route('turma.store') }}" class="turma-form" id="turmaForm">
    @csrf

    @if(!empty($alunoclasse) && $alunoclasse->first())
        <input type="hidden" name="classeFrequentada" value="{{ $alunoclasse->first()->classe_id }}">
        <input type="hidden" name="Anolectivo_IDD" value="{{ $alunoclasse->first()->ano_lectivo_id }}">
    @endif
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-0">
                    <table class="table table-hover table-striped mb-0" id="tabelaAlunosDisponiveis" style="width:100%">
                        ...
                        <tbody>
                            @foreach($alunoclasse as $aluno)
                                <tr data-aluno-id="{{ $aluno->aluno_classe_id }}">
                                    <td>
                                        <input type="hidden" class="aluno-id" value="{{ $aluno->aluno_classe_id }}">
                                        {{ $aluno->nome }}
                                    </td>
                                    <td>{{ \Carbon\Carbon::parse($aluno->dataNascimento)->age }}</td>
                                    <td>{{ $aluno->sexo }}</td>
        ...
        <div class="col-xl-4 col-md-6 col-sm-12 ">
            <div class="card shadow-sm">
                <div class="card-header bg-success text-white">
                    <strong>{{ $flag == 1 ? 'Nova Turma' : 'Novo Jurri' }}</strong>
                </div>
                <div class="card-body">
                    <div class="form-group mb-3">
                        <label for="nomeTurma" class="form-label">
                            {{ $flag == 1 ? 'Nome da Turma' : 'Nome do Jurri' }}
                        </label>
                        <input type="text"
                               name="nomeTurma"
                               id="nomeTurma"
        ...
        <div class="col-xl-4 col-md-6 col-sm-12 ">
            <div class="card shadow-sm">
                <div class="card-header bg-secondary text-white">
                    <strong>{{ $flag == 1 ? 'Turmas Criadas' : 'Jurris Criados' }}</strong>
                </div>
                <div class="card-body p-0" style="max-height: 500px; overflow-y: auto;">
                            ...
                            @foreach($turmascriadas as $item)
                                <div class="list-group-item">
                                    <div class="d-flex justify-content-between align-items-center turma-item"
                                         data-id="{{ $item->turma_id ?? $item->jurri_id }}">
                                        <span class="fw-bold">{{ $item->turma ?? $item->jurri }}</span>
                                        <i class="fa fa-chevron-down text-secondary toggle-options"
                                           role="button"
                                           style="cursor: pointer;"></i>
                                    </div>
                                    <div class="turma-options mt-2 d-none" data-id="{{ $item->turma_id ?? $item->jurri_id }}">
                                        <div class="btn-group w-100" role="group">
                                            <button type="button" class="btn btn-sm btn-outline-primary editar-turma">
                                                <i class="fa fa-edit"></i> Editar
                                            </button>
                                            <a href="{{ route('turma.show', [$item->turma_id ?? $item->jurri_id, 2, $flag]) }}"
                                            ...
                                            <a href="{{ route('turma.show', [$item->turma_id ?? $item->jurri_id, 1, $flag]) }}"
                    ...
                    @else
                        <div class="text-center text-muted py-4">
                            ...
                            <p>Nenhum {{ $flag == 1 ? 'turma' : 'jurri' }} criado.</p>
                        </div>
                    @endif
                </div>
            </div>
...
$(document).ready(function() {
    ...
    window.flagTurma = @json($flag);
    window.routesTurma = {
        store: "{{ route('turma.store') }}",
        update: "{{ route('turma.update', ':id') }}",
        edit: "{{ route('turma.edit', ':id') }}",
        show: "{{ route('turma.show', [':id', ':type', ':flag']) }}"
    };
...
// adicionar de forma dinamica
   ...
   var area = "";
    ...
    $.ajax({
        url: "{{ Route('turma.criaturmapersonalisada') }}",
        type: "POST",
        data: {
selectIdade: $(".por_idade:checked").val(),
            ...
            "_token": "{{ csrf_token() }}" // Ensure the CSRF token is properly used,
           // dadosTurma: allData
        },
        success: function(data) {
